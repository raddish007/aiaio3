...
export default function WishButtonRequestRefactored() {
  ...
  const generateStoryVariables = async (child: Child) => {
    ...
    try {
      // This is a simplified version - you'd implement the actual API call here
      const variables: StoryVariables = {
        childName: child.name,
        theme: child.theme || child.primary_interest,
        visualStyle: 'whimsical cartoon',
        mainCharacter: child.name,
        sidekick: 'magical companion',
        wishResultItems: 'toys and treats',
        buttonLocation: 'enchanted forest',
        magicButton: 'glowing golden button',
        chaoticActions: 'everything spinning wildly',
        realizationEmotion: 'thoughtful understanding',
        missedSimpleThing: 'saying please',
        finalScene: 'peaceful garden scene'
      };
      
      setStoryVariables(variables);
      const initialAssets = initializeAssets(variables);
      setAssets(initialAssets);
      
      // Create or update story project
      await createOrUpdateStoryProject(child, variables);
    } catch (error) {
  ...
  const generateAllPrompts = async () => {
    ...
    setPromptProgress({ current: 0, total: 9, currentPage: '' });
    
    try {
      // This is simplified - implement your actual prompt generation logic
      const prompts: { [key: string]: { image: string; audio: string; safeZone: string } } = {};
      
      for (let i = 1; i <= 9; i++) {
        setPromptProgress({ current: i, total: 9, currentPage: `page${i}` });
        
        prompts[`page${i}`] = {
          image: `Generated image prompt for page ${i}`,
          audio: `Generated audio prompt for page ${i}`,
          safeZone: `Safe zone description for page ${i}`
        };
  ...
  const generateAudio = async (page: string) => {
    ...
    console.log(`Generating audio for ${page}`);
  };
      ...
      case 'audio':
        ...
        return (
          <AssetManagement
            ...
            onSetExpandedPrompt={setExpandedPrompt}
