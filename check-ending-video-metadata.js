...
async function checkEndingVideoMetadata() {
  ...
  console.log('Video Type:', specificVideo.video_type);
  ...
  const { data: letterVideos, error: letterError } = await supabase
    .from('assets')
    .select('id, title, theme, video_type, purpose, status')
    .like('theme', 'Letter %')
    .order('theme');
  ...
  Object.keys(groupedByTheme).sort().forEach(theme => {
    ...
    groupedByTheme[theme].forEach(video => {
      console.log(`  - ${video.id}: ${video.title} (${video.video_type}, purpose: ${video.purpose || 'null'})`);
    });
  });
