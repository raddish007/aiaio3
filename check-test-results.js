require('dotenv').config({ path: '  // Check content_projects table too
  const { data: projects, error: projectError } = await supabaseAdmin
    .from('content_projects')
    .select('*')
    .eq('metadata->>template', 'wish-button')
    .order('created_at', { ascending: false});ocal' });
...
async function checkAfterTest() {
  ...
  // Check content_projects table too
  const { data: projects, error: projectError } = await supabaseAdmin
    .from('content_projects')
    .select('*')
    .eq('template', 'wish-button')
    .order('created_at', { ascending: false });
  ...
  if (projects.length > 0) {
    console.log('✅ Latest project:', {
      ...
      child_name: projects[0].child_name,
